...
export interface PlaybookData {
	 ...
	 *
	 * Each user may only have one playbook with a given global ID, but multiple
	 * users could each have a copy of a playbook with the same global ID.
	 */
	globalID: UUID;
	...
	/**
	 * Global ID for the image file used in the playbook cover. `null` if the
	 * playbook doesn't have a cover image.
	 */
	coverImageFileGlobalID: UUID | null;

	/**
	 * Global ID for the image file used in the playbook banner. `null` if the
	 * playbook doesn't have a banner image.
	 */
