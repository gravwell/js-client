...
export const makeSubscribeToOneExplorerSearch = (
	context: APIContext,
): ((
	query: Query,
	options?: { filter?: SearchFilter | undefined; metadata?: RawJSON | undefined; noHistory?: boolean | undefined },
	...
	return async (
		...
		const entries$: Observable<ExplorerSearchEntries> = searchMessages$.pipe(
			...
			map((msg): ExplorerSearchEntries => {
				...
				const searchEntries = { ...base, filter: filterMap } as SearchEntries;
